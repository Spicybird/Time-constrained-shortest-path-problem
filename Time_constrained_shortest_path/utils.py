...
def output_path_finding_results(agent):
    ...
    with open(output_dir + '/output_path.csv', 'w', newline='') as fp:
...
def output_gap_info(iter_list, g_ub_list, g_lb_list, lag_multiplier_list):
    if not os.path.isdir('./output'):
        os.mkdir('./output')
    output_dir = './output'

    with open(output_dir + "/output_gap.csv", "w", newline="") as fp:
        writer = csv.writer(fp)
        line = ['iter_num', 'global_ub', 'global_lb', 'absolute_gap', 'relative_gap', 'lagrangian_multiplier']
        writer.writerow(line)
        for i in range(len(iter_list)):
            line = []
            line.append(iter_list[i])
            line.append(g_ub_list[i])
            line.append(g_lb_list[i])
            line.append(g_ub_list[i] - g_lb_list[i])
            line.append(100 * (g_ub_list[i] - g_lb_list[i]) / g_ub_list[i])
            line.append(lag_multiplier_list[i])
            writer.writerow(line)
